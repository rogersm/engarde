...
(defvar prolog-mode-version "1.25rogersm"
...
(add-hook 'prolog-mode-hook '(lambda () (substitute-patterns-with-unicode
                                    (list
				     (cons "\\(:-\\)" 'left-arrow)   ; :-
				     (cons "\\(=:=\\)" 'equal)       ; =:=
				     (cons "\\(=\\\\=\\)" 'not-equal)   ;  =\=
...
;;;
  ...
  or GREATER-THAN into an actual Unicode character code. "
    (decode-char 'ucs (case name
                        ;; arrows
                        ('left-arrow 8592) ; 10229 	LONG LEFTWARDS ARROW
                        ('up-arrow 8593)
                        ('right-arrow 8594)
                        ('down-arrow 8595)
                        ;; boxes
